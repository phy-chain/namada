...
pub fn update_db_keys(
    ...
    for (key, value) in updates {
        tracing::debug!("Overwriting key <{}> with value: {}...", key, value);
        let data = HEXUPPER.decode(value.as_bytes())
            .expect("Could not decode a hex value into bytes");
        db.overwrite_entry(&mut batch, None, &key, data)
