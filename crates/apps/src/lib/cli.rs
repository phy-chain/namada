...
pub mod cmds {
    ...
    #[derive(Clone, Debug)]
    pub struct LedgerUpdateDB (args::LedgerUpdateDb);
    ...
    impl SubCmd for LedgerUpdateDB {
        ...
        fn def() -> App {
            App::new(Self::CMD).about(
                "Applies a set of updates to the DB for hard forking. The input should be a path \
                to a file dictating a set of keys and their new values. Can be dry-run for testing.",
            )
        }
    }
    ...
    impl Args for LedgerUpdateDb {
        ...
        fn def(app: App) -> App {
            ...
            .arg(DRY_RUN_TX.def().help(
                "If set, applies the updates but does not persist them. Using for testing and debugging.",
            ))
            .arg(FORCE.def().help(
                "Apply changes even if validation does not pass.",
            ))
        }
    }
